...
-(void)getCountryInfo{
    ...
    NSString *URLString = [NSString stringWithFormat:@"http://192.168.1.20/api/search/countries/name/%@", self.txtCountry.text];
    ...
    [AppDelegate downloadDataFromURL:url withCompletionHandler:^(NSData *data) {
        ...
        if (data != nil) {
            ...
            // cris
            self.countryDetailsDictionary = [[returnedDict objectForKey:@"entities"] objectAtIndex:0];
            NSLog(@"lowerCaseString is: %@", self.countryDetailsDictionary);
            self.lblCountry.text = [NSString stringWithFormat:@"%@ (%@)", [self.countryDetailsDictionary objectForKey:@"code"], [self.countryDetailsDictionary objectForKey:@"name"]];
...
//            }
//            else{
//                self.countryDetailsDictionary = [[returnedDict objectForKey:@"code"] objectAtIndex:0];
//                
...
- (IBAction)sendJSON:(id)sender {
    ...
    NSDictionary *dictionary = @{@"countryName": [self.countryDetailsDictionary objectForKey:@"entities[0].name"],
                                 @"countryCode": [self.countryDetailsDictionary objectForKey:@"entities[0].code"]};
